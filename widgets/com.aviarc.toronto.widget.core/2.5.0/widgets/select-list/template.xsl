<xsl:stylesheet
    ...
    <xsl:template match="select-list">
        ...
        <div id="{@name}:div" class="{cssutils:makeClassString(concat($css-prefix, 'select-list'), $class)} {$invisible-class}" title="{@tooltip}">
            <xsl:choose>
                <xsl:when test="$enabled = 'y'">
                    <select id="{@name}:input" size="{$rows}" title="{@tooltip}"></select>
                </xsl:when>
                <xsl:otherwise>
                    <select id="{@name}:input" size="{$rows}" disabled="true" title="{@tooltip}"></select>
                </xsl:otherwise>
            </xsl:choose>
